...
void MainWindow::on_volumeRenderingSelectDatFilePushButton_clicked()
{
    ...
    if (!visualizationPtr->m_datFilePath.empty())
    {
        ui->volumeRenderingDataRawInfoLabel->setText("Loading " + QString::fromStdString(visualizationPtr->m_datFilePath));
        visualizationPtr->opengl_loadDataRawFromFile();

        if (!ui->volumeRenderingDataRawRadioButton->isChecked())
